...
namespace Qos.Service.Tests.QSeries;
...
public class QSeriesTransportStoreTests : IDisposable
{
    ...
    {
        _tempDir = Path.Combine(Path.GetTempPath(), "qos-qseries-store-tests-" + Guid.NewGuid().ToString("N"));
    ...
    public void Save_creates_parent_directory_if_missing()
    {
        ...
        // %ProgramData%\Qos doesn't exist yet).
