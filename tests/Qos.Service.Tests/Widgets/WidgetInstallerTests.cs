...
/// %APPDATA% / ~/Library/Application Support/Qos directory.
/// </summary>
public class WidgetInstallerTests : IDisposable
{
    ...
    {
        _bundledRoot = Path.Combine(Path.GetTempPath(), "qos-installer-bundled-" + Guid.NewGuid().ToString("N")[..8]);
        _userRoot = Path.Combine(Path.GetTempPath(), "qos-installer-user-" + Guid.NewGuid().ToString("N")[..8]);
