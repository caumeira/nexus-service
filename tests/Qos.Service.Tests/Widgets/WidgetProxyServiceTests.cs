...
public class WidgetProxyServiceTests : IDisposable
{
    private readonly string _root;
    private readonly WidgetRegistry _registry;

    public WidgetProxyServiceTests()
    {
        _root = Path.Combine(Path.GetTempPath(), "qos-proxy-tests-" + Guid.NewGuid().ToString("N")[..8]);
    ...
    private WidgetProxyService MakeService(HttpMessageHandler handler)
    {
        var factory = new StubHttpFactory(new HttpClient(handler));
        return new WidgetProxyService(factory, _registry);
    }
    ...
    public async Task Caps_response_body_at_1_MiB()
    {
        ...
        Assert.True(resp.BodyText!.Length <= WidgetProxyService.MaxBodyBytes);
    }
    ...
    public async Task Sets_default_user_agent_when_widget_does_not_supply_one()
    {
        ...
        Assert.Contains("Qos-Widget-Proxy", ua);
    }
    ...
    public async Task Widget_supplied_user_agent_overrides_the_default()
    {
        ...
        Assert.DoesNotContain("Qos-Widget-Proxy", ua);
    }
