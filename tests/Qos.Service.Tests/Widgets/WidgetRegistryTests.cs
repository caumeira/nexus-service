...
public class WidgetRegistryTests : IDisposable
{
    ...
    {
        _root = Path.Combine(Path.GetTempPath(), "qos-widgets-tests-" + Guid.NewGuid().ToString("N")[..8]);
    ...
    public void Discovers_fixture_widget()
    {
        ...
        Assert.Equal("qos.widget/2", entry.Manifest.Schema);
    ...
    public void First_root_wins_when_id_appears_in_multiple_roots()
    {
        ...
        var secondRoot = Path.Combine(Path.GetTempPath(), "qos-widgets-tests-2-" + Guid.NewGuid().ToString("N")[..8]);
