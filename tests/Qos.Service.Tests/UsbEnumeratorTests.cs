...
public class UsbEnumeratorTests
{
    ...
    {
        var tempRoot = Path.Combine(Path.GetTempPath(), $"qos-usb-test-{Path.GetRandomFileName()}");
        try
        {
    ...
    {
        var tempRoot = Path.Combine(Path.GetTempPath(), $"qos-usb-speed-{Path.GetRandomFileName()}");
        try
        {
            string[][] cases =
            {
