...
public abstract class AppUsageHistorySpec : IDisposable
{
    ...
    {
        Store.Append(new[] { StorageTick(1000, ("app.exe", 4_194_304)) }, null);

        var top = Store.QueryTopApps("storage", 0, 10_000, 15);
    ...
    {
        Store.Append(new[] { StorageTick(2000, ("app.exe", 2000)), StorageTick(1000, ("app.exe", 1000)) }, null);

        var points = Store.QueryAppSeries("storage", "app.exe", 0, 10_000);
    ...
    {
        Store.Append(new[] { StorageTick(1000, ("low", 500), ("high", 90_000), ("mid", 40_000)) }, null);

        var top = Store.QueryTopApps("storage", 0, 10_000, 2);
    ...
    {
        Store.Append(new[] { StorageTick(1000, ("Chrome.exe", 1000)), StorageTick(1005, ("chrome.exe", 3000)) }, null);
    ...
    {
        Store.Append(new[] { StorageTick(1000, ("app.exe", 1000)), StorageTick(5000, ("app.exe", 2000)) }, null);
    ...
    public void Append_WithPruneCutoff_KeepsAnAppSeriesRow_ForAnAppSeenOnlyInStorage()
    {
        ...
        // QueryTopApps' JOIN.
        Store.Append(new[] { StorageTick(9000, ("storage-only.exe", 1000)) }, null);
    ...
    {
        Store.Append(new[] { CpuTick(9000, ("app.exe", 10)) }, null);
        Store.Append(new[] { StorageTick(2000, ("app.exe", 1000)) }, null);
