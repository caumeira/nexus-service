...
/// <summary>
/// MiniHub ports 3 and 4 are raw ARGB headers, the same problem as a Smart
/// Hub port: the firmware cannot say what is daisy-chained to them, so the
/// user declares the chain and each product becomes its own zone. The card
/// list, the engine frames, and the bytes the writer lays into the port
/// buffer all read these zones, so they have to agree.
/// </summary>
public class MiniHubChainTests
{
    private const string HubId = "minihub:MH01";
    ...
    /// <summary>FR12 Trio + Y50 Solo Fan on port 3: 68 + 8 = 76 declared LEDs.</summary>
    ...
    public void An_unchained_port_is_one_zone_carrying_the_port_id()
    {
        ...
        var zone = Assert.Single(MiniHubLightingDeviceProvider.ResolveLedPortZones(settings, HubId, Channel, 0));
    ...
    public void The_declared_count_wins_over_the_firmware_report()
    {
        ...
        // The hub reports 0 for this port until the user says otherwise.
        var zone = Assert.Single(MiniHubLightingDeviceProvider.ResolveLedPortZones(settings, HubId, Channel, 0));
    ...
    {
        var zones = MiniHubLightingDeviceProvider.ResolveLedPortZones(Chained(), HubId, Channel, 0);
    ...
    {
        var structure = MiniHubLightingDeviceProvider.BuildLedPortStructure(Chained(), HubId, Channel, 0);
    ...
    public void A_port_only_counts_as_uncontrolled_when_every_product_does()
    {
        ...
        var zones = MiniHubLightingDeviceProvider.ResolveLedPortZones(settings, HubId, Channel, 0);
    ...
    {
        var settings = Chained();
        var structure = MiniHubLightingDeviceProvider.BuildLedPortStructure(settings, HubId, Channel, 0);
