...
- **Elgato Stream Deck** - native gen1 HID transport (button-only models; bench-verified on the Mini) over the existing HID stack (`Peripherals/StreamDeck/`), exposed as a first-party device with the full `/streamdeck/*` binding/config/image/dispatch contract (`Routes/StreamDeckRoutes.cs`) plus bench/dev routes (test-pattern, DEV_TOOLS sim-press). Bindings share the same `DeckAction` union as the panel's Deck widget (`src/Deck/`), dispatched headlessly by `DeckActionExecutor`; gen2 (JPEG) models and the web editor land in later phases.
