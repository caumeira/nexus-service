...
The local Qos hardware service. One Native-AOT binary that runs on the user's PC (Windows) or Mac, talks to the hardware, and exposes a REST + WebSocket API that the [`qos-web`](https://github.com/nexusqos/qos-web) dashboard, the on-device panels, and [`qos-overlay`](https://github.com/nexusqos/qos-overlay) all consume.

This is the engine of [Qos](https://nexusqos.com). The other repos are clients of this one.
...
- **Widgets** — host for the `qos.widget/2` SDK widgets shipped in [`qos-widgets`](https://github.com/nexusqos/qos-widgets), with declarative views, sensor bindings, and (when capability-granted) a sandboxed Web Worker.
...
src/
  ...
  Widgets/            # qos.widget/2 host (manifest loader, data sources, worker sandbox)
...
Bundled/
  win-x64/openrgb/    # OpenRGB-headless.exe + DLLs (from qos-rgb fork)
...
installer/
  Qos.iss             # Inno Setup script
  build-installer.ps1 # Windows installer assembly
tests/
  Qos.Service.Tests   # xUnit, AOT-safe
```
...
Both publishes fail loudly if the bundled OpenRGB binaries aren't present under `Bundled/{rid}/openrgb/`. Build them from [`qos-rgb`](https://github.com/nexusqos/qos-rgb) first — error messages from the csproj spell out the exact commands.

The Windows installer (`Qos-Setup.exe`) is produced separately, after the publish:

```ps1
powershell -File installer/build-installer.ps1
```

The Mac DMG is produced from `publish-mac/` via the master `qos` repo's release scripts.
...
Installer artifacts are published to [`nexusqos/qos-releases`](https://github.com/nexusqos/qos-releases) as `Qos-Setup.exe` and `Qos.dmg` under monotonic `vNN` tags. The download links on nexusqos.com point at `/releases/latest/download/<asset>`.
