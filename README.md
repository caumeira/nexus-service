...
- **App store** - downloadable SDK apps from the cloud catalog. `Store/StoreCatalogProxy` fetches the listing from api.hellonexus.com and rewrites asset URLs onto `/apps-api/store/media/*`, because the panel's CSP is `default-src 'self'` and would block both the cross-origin fetch and the images. `Store/StoreInstaller` composes the artifact URL itself from the app id and version rather than trusting a URL from the response, downloads it with the catalog's sha256 and size pinned, and extracts into a staging dir that is moved into place only after the unpacked `manifest.json` agrees on id and version. Routes: `GET /apps-api/store/apps`, `GET /apps-api/store/apps/{appId}`, `GET /apps-api/store/media/{**path}`, `POST /apps-api/store/install` (`Routes/AppRoutes.cs`).
...
src/
  ...
  Store/              # downloadable SDK apps: catalog proxy (CSP forces listing + media through the service) and the artifact installer (hash/size pinned, staged then moved)
