...
- **Game Mode** - while a catalog game's process is running (or switched on by hand), native notifications are queued for release afterwards and periodic cloud egress is deferred: heartbeat, telemetry flush, fleet retries, fps upload, OTA polling, cloud device reports. Two opt-in extras put every Nexus-driven panel display to sleep and stop panel rendering. Tracks the game's PROCESS, not its focus session, so alt-tabbing does not toggle it (`src/Games/GameModeState.cs`, `GET`/`POST /api/game-mode`, live on the `gameMode` WebSocket topic).
...
src/
  ...
  Store/              # downloadable SDK apps: cloud catalog proxy (rewrites media onto a same-origin route, which img-src requires) and the artifact installer (hash-verified, staged then moved)
