        ...
        for (int i = 0; i < devices.Count; i++)
        {
            ...
            if (isDefault && !isSplitMotherboard)
            {
                ...
                result.Add(new LightingDevice
                {
                    ...
                    LedCount = d.LedCount,
                    EnabledLedCount = ZoneResolution.CountEnabled(structure, zones[0], baseId, d.LedCount, settings),
                    CanvasX = layout?.X ?? dx,
                    CanvasY = layout?.Y ?? dy,
                    CanvasW = layout?.W ?? dw,
                    CanvasH = layout?.H ?? dh,
            ...
            if (isDefault)
            {
                ...
                for (int z = 0; z < d.Zones.Count; z++)
                {
                    ...
                    result.Add(new LightingDevice
                    {
                        ...
                        EnabledLedCount = ZoneResolution.CountEnabled(structure, zones[z], zoneId, effectiveLedCount, settings),
                        CanvasX = layout?.X ?? sx,
                        CanvasY = layout?.Y ?? sy,
                        CanvasW = layout?.W ?? sw,
                        CanvasH = layout?.H ?? sh,
            ...
            foreach (var zone in zones)
            {
                ...
                result.Add(new LightingDevice
                {
                    ...
                    EnabledLedCount = ZoneResolution.CountEnabled(structure, zone, zone.Id, zone.LedCount, settings),
                    CanvasX = layout?.X ?? zx,
                    CanvasY = layout?.Y ?? zy,
                    CanvasW = layout?.W ?? zw,
                    CanvasH = layout?.H ?? zh,
